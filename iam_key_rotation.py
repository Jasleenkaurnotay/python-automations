...
def delete_key(user_key_date):
    for u in user_key_date:
        try:
            if u[2] > key_age:
                logging.info("Deleting access key {} for user {}".format(u[0], u[1]))
                key_delete = iam_client.delete_access_key(
                    UserName=u[0],
                    AccessKeyId=u[1]
                )
                logging.info("Successfully deleted access key {} for user {}".format(u[0], u[1]))
