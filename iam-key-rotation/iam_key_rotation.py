        ...
        try:
            if u[2] > key_age:          # Only expired keys would be rotated
                try:
                    ...
                    print(e)
                    logger.info(f"Deleting access key for {u[0]} because AccessKeyLimit reached")
                    iam_client.delete_access_key(
                        UserName=u[0],
                        AccessKeyId=u[1]
                    )
                    logger.info(f"User access key deleted, attempting creation of new key")
                    ak = iam_client.create_access_key(
                        UserName=u[0]
                    )
                    logger.info("Access key and secret access key created successfully for user: {}".format(u[0]))
...
def store_aws_credentials(updated_user_keypair):
    for uuk in updated_user_keypair:
        ...
        try:
            # To make sure for an existing user and secret, this block doesnt fail
            Name="rotated_key_pair_{}".format(uuk[0])
            try:
                sm_client.describe_secret(SecretId=Name)
                print(f"Secert with the name: {Name} exists, updating its value")
                sm_client.put_secret_value(
                    SecretId=Name,
                    SecretString=json.dumps(credential_data)
                )
            except sm_client.exceptions.ResourceNotFoundException as error:
                print(error)
